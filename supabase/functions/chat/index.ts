...
1. המשתמש בוחר שעה ספציפית → הגדר \`fixed_start\` לשעה שנבחרה → המשימה תופיע אוטומטית בלוח.
2. המשתמש מביע כוונה שה-AI יתזמן / יסדר / ישבץ / יקבע — **הבן את הכוונה מהקשר**, לא ממילים מדויקות. כל ביטוי של "סדר לי", "שבץ", "קבע", "תחליט", "תתכנן", "תשים", "תסדר את זה", "בסדר", "טוב", "כן" (בתגובה לשאלת התזמון) — **קרא מיד ל-\`create_task\` ואז ל-\`generate_schedule\` עם תאריך המשימה. לעולם אל תדחה את המשתמש ותגיד לו ללחוץ "בנה את היום" — קרא ל-generate_schedule בעצמך.**
3. המשתמש מביע בפירוש רצון לדחות ("בהמשך", "אחר כך", "לא עכשיו", "תשאיר בינתיים") — צור ללא \`fixed_start\`. **בחר אפשרות זו רק כשהמשתמש אומר בבירור שהוא רוצה לדחות — ברירת המחדל היא אפשרות 2.**

**הצגה בלוח הזמנים:** משימה עם \`fixed_start\` מופיעה אוטומטית בלוח ביום שלה — **אין צורך ב-"בנה את היום"**. לאחר קריאה ל-\`generate_schedule\`, הלוח מתעדכן אוטומטית. משימה **ללא** \`fixed_start\` ובלי generate_schedule מוצגת רק לאחר הפעלת "בנה את היום". ציין זאת בתשובה למשתמש.
